...
def copy_request_inputs(request_json_path, image_abs_path, request_folder):
    ...
    # copy input image with original extension
    _, ext = os.path.splitext(image_abs_path)
    image_copy = os.path.join(request_folder, f"input_image{ext}")
    shutil.copy2(image_abs_path, image_copy)

    return request_json_copy, image_copy
...
def copy_generated_outputs(quote_result, request_folder):
    ...
    if output_files.get("bom_csv") and os.path.exists(output_files["bom_csv"]):
        dst = os.path.join(request_folder, "scaled_bom.csv")
        shutil.copy2(output_files["bom_csv"], dst)
        copied_paths["scaled_bom"] = dst

    # Cost CSV
    if output_files.get("cost_csv") and os.path.exists(output_files["cost_csv"]):
        dst = os.path.join(request_folder, "quotation_cost.csv")
        shutil.copy2(output_files["cost_csv"], dst)
        copied_paths["quotation_cost"] = dst

    # Summary JSON
    if output_files.get("summary_json") and os.path.exists(output_files["summary_json"]):
        dst = os.path.join(request_folder, "quotation_summary.json")
        shutil.copy2(output_files["summary_json"], dst)
        copied_paths["quotation_summary"] = dst

    # Fusion scaled components
...
def main():
    ...
    # If user gives relative path, resolve it from project root
    if not os.path.isabs(request_json_path):
        request_json_path = os.path.join(project_root, request_json_path)
    ...
    # Copy request + image into request folder first
    request_json_copy, image_copy = copy_request_inputs(
        request_json_path,
        image_abs_path,
        request_folder
    )
